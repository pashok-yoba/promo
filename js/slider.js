...
function mpSliderBI(manual, first)
{
    ...
    $('.banner-item').on('touchstart', function(e){
        xPosBegin = e.originalEvent.touches[0].pageX;
    });
    $('.banner-item').on('touchmove', function(e){
        xPosEnd =  e.originalEvent.touches[0].pageX;

    });
    $('.banner-item').on('touchend', function(e){
        if ( xPosEnd > xPosBegin ) {
            console.log('left');
            $('#prm-arrow-right').trigger('click');
        }
        if ( xPosEnd < xPosBegin) {
            console.log('right');
            $('#prm-arrow-left').trigger('click');
        }
    });
    ...
    if(!this.manual){
        this.interval = setInterval(newSlide(), 4500);
        $('.banner-main-container').hover(
            function(){target.mouseover = true; clearInterval(target.interval);},
            function(){target.mouseover = false; target.interval = setInterval(newSlide(), 4500)}
        );
    }
}
